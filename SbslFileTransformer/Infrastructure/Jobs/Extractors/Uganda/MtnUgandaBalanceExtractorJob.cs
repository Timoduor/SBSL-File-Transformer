...
{
    public class MtnUgandaBalanceExtractorJob : ConverterJobBase<GLBalanceExtractorJob>, IHostedService
    {
        protected override string JobName { get; set; } = nameof(MtnUgandaBalanceExtractorJob);

        public MtnUgandaBalanceExtractorJob(IServiceScopeFactory serviceScopeFactory, ILogger<GLBalanceExtractorJob> logger, JobDisplayManager jobManager)
        {
        ...
        private async Task ExtractMultiCurrBalances()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Mtn Uganda Balance Extractor job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    MtnUgandaBalanceExtractor mpesaConverter = new MtnUgandaBalanceExtractor();
                    ...
                    {
                        if (file.ToLower().Contains("imug") && file.ToLower().Contains("mobile_banking") && file.ToLower().Contains("mtnw2b"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.BalanceExtracted == false)
                                ...
                                finally
                                {
                                    ...
                                    fileToProcess.ConvertedBy = nameof(MtnUgandaBalanceExtractor);

                                    updatedFiles.Add(fileToProcess);
                                }
                        }
                        else if (file.ToLower().Contains("imug") && file.ToLower().Contains("mobile_banking") && file.ToLower().Contains("mtnb2w"))
                        {
                            SftpUploadedFile fileToProcess =
                                uploadedFiles.FirstOrDefault(f =>
                                    f.FilePath.ToLower() == file.ToLower());
