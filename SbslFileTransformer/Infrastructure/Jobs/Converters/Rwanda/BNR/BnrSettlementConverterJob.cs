...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda.BNR
{
    public class BnrSettlementConverterJob : ConverterJobBase<BnrSettlementConverterJob>, IHostedService
    {
        protected override string JobName { get; set; } = nameof(BnrSettlementConverterJob);
        public BnrSettlementConverterJob(ILogger<BnrSettlementConverterJob> logger,
        ...
        {
            this._logger.LogInformation("Starting BNR Settlement Converter Job");
        ...
        private async Task ConvertBnrFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running BNR Settlement converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    SettlementConverter bnrConverter = new SettlementConverter();
                    ...
                    foreach (string file in files)
                    {
                        if (file.ToLower().Contains("bnr") && file.ToLower().Contains("imrw") &&
                            file.ToLower().Contains("settlements"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(SettlementConverter));
                                }
                        }
                    }
