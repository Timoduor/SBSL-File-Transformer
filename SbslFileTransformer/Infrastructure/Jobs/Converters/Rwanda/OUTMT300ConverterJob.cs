...
{
    public class OUTMT300ConverterJob : ConverterJobBase<OUTMT300ConverterJob>, IHostedService
    {
        protected override string JobName { get; set; } = nameof(OUTMT300ConverterJob);
        public OUTMT300ConverterJob(ILogger<OUTMT300ConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            this._logger = logger;
        ...
        {
            this._logger.LogInformation("Starting OUTMT300 Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertOutMT300File(), null, TimeSpan.FromSeconds(new Random().Next(60, 200)), TimeSpan.FromMinutes(10));

            return Task.CompletedTask;
        }

        private async Task ConvertOutMT300File()
        {
            try
            {
                await _semaphore.WaitAsync();

                this._logger.LogInformation("Running OUTMT300 Converter Job");
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("mt300") && file.ToLower().Contains("imrw"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(OUTMt300Converter));
                                }
