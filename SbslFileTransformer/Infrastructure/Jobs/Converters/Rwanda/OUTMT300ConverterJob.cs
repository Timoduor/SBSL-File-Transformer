...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters
{
    public class OUTMT300ConverterJob : ConverterJobBase<OUTMT300ConverterJob>, IHostedService
    {
        ...
        {
            _logger.LogInformation("Starting OUTMT300 Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            _timer = new Timer(async state => await ConvertOutMT300File(), null, TimeSpan.FromSeconds(new Random().Next(60, 200)), TimeSpan.FromMinutes(10));
        ...
        private async Task ConvertOutMT300File()
        {
            try
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    var mt300Converter = new OUTMt300Converter();
                    ...
                    foreach (var file in files)
                    {
                        ...
                        if (file.ToLower().Contains("mt300") && file.ToLower().Contains("imrw"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    CompleteFileProcessing(updatedFiles, fileToProcess, nameof(OUTMt300Converter));
                                }
