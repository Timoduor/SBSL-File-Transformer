...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class Camt053ConverterJob : ConverterJobBase<Camt053ConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting CAMT053 Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertCamtFile(), null,
        ...
        private async Task ConvertCamtFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running CAMT053 Converter Job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    List<string> files = Directory.GetFiles(prodFolder, "*.xml", options).ToList();

                    files.AddRange(Directory.GetFiles(sbFolder, "*.xml", options));

                    Camt053Converter camtConverter = new Camt053Converter();
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("camt053") && file.ToLower().Contains("imrw"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(Camt053Converter));
                                }
                        }
                    }
