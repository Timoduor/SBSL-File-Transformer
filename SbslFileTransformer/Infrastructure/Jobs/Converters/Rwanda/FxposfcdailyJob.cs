...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class FxposfcdailyJob : ConverterJobBase<FxposfcdailyJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Fxposfcdaily  Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.FxposfcdailyConverter(), null,
        ...
        private async Task FxposfcdailyConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Fxposfcdaily Converter RW job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    Fc_dailyConverter fc_Converter = new Fc_dailyConverter();
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("imrw") && file.ToLower().Contains("treasury") && file.ToLower().Contains("fx_pos") && file.ToLower().Contains("fx_pos_fc_daily"))//&& !file.ToLower().Contains("conv")
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(Fc_dailyConverter));
                                }
                        }
                    }
