...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters
{
    public class AirtelRwandaBalanceExtractorJob : ConverterJobBase, IHostedService
    {
        ...
        {
            _logger.LogInformation("Starting Airtel RW Balance Extractor Job");
        ...
        private async Task AirtelFileBalanceExtractor()
        {
            try
            {
                ...
                _logger.LogInformation("Running Airtel RW Balance Extractor job");
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    var mpesaConverter = new AirtelRwandaBalanceExtractor(Entity);

                    foreach (var file in files)
                    {
                        if (file.ToLower().Contains("airtel") && file.ToLower().Contains("b2w") && file.ToLower().Contains("mb") && file.ToLower().Contains("imrw") && file.ToLower().Contains("portal"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                            {
                                ...
                                finally
                                {
                                    ...
                                    fileToProcess.ConvertedBy = nameof(AirtelRwandaBalanceExtractor);
