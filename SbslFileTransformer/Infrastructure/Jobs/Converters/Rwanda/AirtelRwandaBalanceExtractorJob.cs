...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class AirtelRwandaBalanceExtractorJob : ConverterJobBase<AirtelRwandaBalanceExtractorJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Airtel RW Balance Extractor Job");
        ...
        private async Task AirtelFileBalanceExtractor()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Airtel RW Balance Extractor job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    AirtelRwandaBalanceExtractor mpesaConverter = new AirtelRwandaBalanceExtractor(Entity);
                    ...
                    {
                        if (file.ToLower().Contains("airtel") && file.ToLower().Contains("b2w") &&
                            file.ToLower().Contains("mb") && file.ToLower().Contains("imrw") &&
                            file.ToLower().Contains("portal"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(AirtelRwandaBalanceExtractor));
                                }
                        }
                    }
