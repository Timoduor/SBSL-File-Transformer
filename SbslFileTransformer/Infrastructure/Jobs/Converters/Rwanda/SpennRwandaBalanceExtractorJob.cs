        ...
        private async Task AirtelFileBalanceExtractor()
        {
            try
            {
                ...
                using (IServiceScope scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    SpennRwandaBalanceExtractor mpesaConverter = new SpennRwandaBalanceExtractor(Entity);
                    ...
                    {
                        if (file.ToLower().Contains("spenn") && file.ToLower().Contains("imrw") &&
                            file.ToLower().Contains("mb") && file.ToLower().Contains("pp") &&
                            file.ToLower().Contains("bal"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    CompleteFileProcessing(updatedFiles, fileToProcess, nameof(SpennRwandaBalanceExtractor));
                                }
                        }
                    }
