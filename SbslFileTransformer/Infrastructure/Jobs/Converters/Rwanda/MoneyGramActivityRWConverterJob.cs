...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    ...
    {
        public MoneyGramActivityRWConverterJob(ILogger<MoneyGramActivityRWConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            _logger = logger;
        ...
        {
            _logger.LogInformation("Starting MoneyGram Activity RW Converter Job");
        ...
        private async Task MoneyGramConverter()
        {
            try
            {
                ...
                _logger.LogInformation("Running MoneyGram Activity RW Converter  job");
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    foreach (var file in files)
                    {
                        ...
                        if (file.ToLower().Contains("mg") && file.ToLower().Contains("imrw") && file.ToLower().Contains("activity") && !file.Contains("Conv"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    await EmailHelpers.SendEmails(dbContext, "Problem Converting  MoneyGram Activity RW files", $"{file} \n\n {ex.Message}", new string[] { file }, _emailSender);
                                }
                                finally
                                {
                                    fileToProcess.Converted = true;

                                    fileToProcess.ConvertedBy = nameof(MoneyGramActivityRWConverter);
