...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class MTNPushPullRwandaBalanceExtractorJob : ConverterJobBase<MTNPushPullRwandaBalanceExtractorJob>,
        IHostedService
    {
        protected override string JobName { get; set; } = nameof(MTNPushPullRwandaBalanceExtractorJob);
        ...
        {
            this._logger.LogInformation("Starting MTN PUSH PULL RW Balance Extractor Job");
        ...
        private async Task AirtelFileBalanceExtractor()
        {
            try
            {
                ...
                this._logger.LogInformation("Running MTN PUSH PULL RW Balance Extractor job");
                    ...
                    foreach (string file in files)
                    {
                            ...
                            file.ToLower().Contains("portal"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(MtnPushPullBalanceExtractor));
                                }
                        }
                    }
