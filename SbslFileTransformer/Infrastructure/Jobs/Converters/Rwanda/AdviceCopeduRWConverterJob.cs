...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    ...
    {
        public AdviceCopeduRWConverterJob(ILogger<AdviceCopeduRWConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            _logger = logger;
        ...
        private async Task MoneyGramConverter()
        {
            try
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    var mpesaConverter = new AdviceKopeduRWConverter();
                    ...
                    foreach (var file in files)
                    {
                        ...
                        if (file.ToLower().Contains("wu") && file.ToLower().Contains("portal") && file.ToLower().Contains("advice") && file.ToLower().Contains("imrw") && !file.Contains("Conv"))
                        {
