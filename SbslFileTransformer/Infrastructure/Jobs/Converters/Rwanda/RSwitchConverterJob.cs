...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class RSwitchConverterJob : ConverterJobBase<RSwitchConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting RSwitch Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.RSwitchConverter(), null,
        ...
        private async Task RSwitchConverter()
        {
            try
            {
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    RSwitchConverter rSwitchConverter_ = new RSwitchConverter();
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.Contains("ATMs-OFF-US") && file.Contains("IMRW") && !file.Contains("Conv"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(this.RSwitchConverter));
                                }
                        }
                    }
