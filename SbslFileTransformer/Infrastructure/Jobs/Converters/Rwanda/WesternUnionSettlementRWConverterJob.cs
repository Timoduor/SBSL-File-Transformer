...
{
    public class WesternUnionSettlementRWConverterJob : ConverterJobBase<WesternUnionSettlementRWConverterJob>, IHostedService
    {
        public WesternUnionSettlementRWConverterJob(ILogger<WesternUnionSettlementRWConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            _logger = logger;
        ...
        private async Task WesternUnionSettlementConverter()
        {
            try
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    {
                        if (!file.Contains("Conv") && file.ToLower().Contains("imrw") && file.ToLower().Contains("wu") && file.ToLower().Contains("settlement") && file.ToLower().Contains("portal"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    await EmailHelpers.SendEmails(dbContext, "Problem Converting Western Union Settlement RW files", $"{file} \n\n {ex.Message}", new string[] { file }, _emailSender);
                                }
                                finally
                                {
