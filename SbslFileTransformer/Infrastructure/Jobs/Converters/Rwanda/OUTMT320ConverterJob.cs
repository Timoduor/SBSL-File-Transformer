...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Rwanda
{
    public class OUTMT320ConverterJob : ConverterJobBase<OUTMT320ConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting OUTMT320 Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertOutMT320File(), null, TimeSpan.FromSeconds(new Random().Next(60, 200)), TimeSpan.FromMinutes(10));
        ...
        private async Task ConvertOutMT320File()
        {
            try
            {
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    OUTMt320Converter mt320Converter = new OUTMt320Converter();
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("mt320") && file.ToLower().Contains("imrw"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(OUTMt320Converter));
                                }
