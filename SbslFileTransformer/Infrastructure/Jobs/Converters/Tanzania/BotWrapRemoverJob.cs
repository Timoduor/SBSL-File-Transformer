...
{
    public class BotWrapRemoverJob : ConverterJobBase<BotWrapRemoverJob>, IHostedService
    {
        protected override string JobName { get; set; } = nameof(BotWrapRemoverJob);

        public BotWrapRemoverJob(ILogger<BotWrapRemoverJob> logger,
        ...
        {
            this._logger.LogInformation("Starting BOT Wrap Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertBotWrapFile(), null,
                TimeSpan.FromSeconds(new Random().Next(60, 200)), TimeSpan.FromMinutes(10));

            return Task.CompletedTask;
        }

        private async Task ConvertBotWrapFile()
        {
            try
            {
                await _semaphore.WaitAsync();

                this._logger.LogInformation("Running BOT Wrap file converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("imtz") && file.ToLower().Contains("nostro")
                            && file.ToLower().Contains("bot") && !file.Contains("Conv"))
                        {
