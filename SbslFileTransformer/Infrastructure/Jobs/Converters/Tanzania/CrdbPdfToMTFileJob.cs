...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Tanzania
{
    public class CrdbPdfToMTFileJob : ConverterJobBase<CrdbPdfToMTFileJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting CRDB PDF To MT File Converter Job");
        ...
        private async Task ConvertPdfToMTFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running CRDB PDF To MT File converter job");
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("crdb") && file.ToLower().Contains("imtz"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(CrdbPdfToMTFilesConverter));
                                }
                        }
                    }
