...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Tanzania
{
    public class DtbPdfToMTFileJob : ConverterJobBase<DtbPdfToMTFileJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting DTB PDF To MT File Converter Job");
        ...
        private async Task ConvertPdfToMTFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running DTB PDF To MT File converter job");
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("dtb") && file.ToLower().Contains("imtz"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(DtbPdfToMTFilesConverter));
                                }
                        }
                    }
