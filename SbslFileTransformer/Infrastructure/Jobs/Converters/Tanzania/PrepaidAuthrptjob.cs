...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Tanzania
{
    public class PrepaidAuthrptjob : ConverterJobBase<PrepaidAuthrptjob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Prepaid Authorization report Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.PrepaidAuthrpt_Converter(), null,
                TimeSpan.FromSeconds(new Random().Next(60, 200)), TimeSpan.FromMinutes(10));

            return Task.CompletedTask;
        }

        private async Task PrepaidAuthrpt_Converter()
        {
            try
            {
                await _semaphore.WaitAsync();

                this._logger.LogInformation("Running Prepaid Authorization report TZ job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    PrepaidAuthrpt_converter PrepaidAuthrpt_conv = new PrepaidAuthrpt_converter();
                    ...
                    foreach (string file in files)
                    {
                        if (file.ToLower().Contains("imtz") && file.ToLower().Contains("cards_atm") && file.ToLower().Contains("floatcms") && file.ToLower().Contains("prepaid") && !file.ToLower().Contains("conv"))// 
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                try
                                {
                                    PrepaidAuthrpt_conv.ConvertFile(file, rootFolder);
                                }
                                catch (Exception ex)
                                {
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(PrepaidAuthrpt_converter));
                                }
                        }
                    }
