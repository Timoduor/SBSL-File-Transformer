...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Tanzania
{
    public class SelcomDisbursementConverterJob : ConverterJobBase<SelcomDisbursementConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Selcom Disbursement Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.SelcomDisbFileConverter(), null,
        ...
        private async Task SelcomDisbFileConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Selcom Disbursement Converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    SelcomDisbConverter mpesaConverter = new SelcomDisbConverter();
                    ...
                    foreach (string file in files)
                    {
                            ...
                            file.ToLower().Contains("selcomdisb"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(SelcomDisbConverter));
                                }
                        }
                    }
