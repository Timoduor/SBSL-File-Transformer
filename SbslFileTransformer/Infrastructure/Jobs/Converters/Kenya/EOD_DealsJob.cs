...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class EOD_DealsJob : ConverterJobBase<EOD_DealsJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting EOD Deals Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.EOD_DealsConverter(), null,
        ...
        private async Task EOD_DealsConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running EOD Deals Converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    EODDealsKEConverter EODDealConverter = new EODDealsKEConverter();
                    ...
                    foreach (string file in files)
                    {
                        ...
                        if (file.ToLower().Contains("gl_entries") && file.ToLower().Contains("imke") && !file.Contains("Conv"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(EODDealsKEConverter));
                                }
                        }
                    }
