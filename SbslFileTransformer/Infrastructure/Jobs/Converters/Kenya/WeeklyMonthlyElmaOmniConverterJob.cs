...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class WeeklyMonthlyElmaOmniConverterJob : ConverterJobBase<WeeklyMonthlyElmaOmniConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Weekly Monthly Elma Omni Settlement Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.WeeklyElmaConverter(), null,
        ...
        private async Task WeeklyElmaConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Weekly Monthly Elma Omni Settlement job");
                    ...
                    foreach (string file in files)
                    {
                            ...
                            (file.ToLower().Contains("elma") || file.ToLower().Contains("omni")))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(WeeklyMonthlyElmaOmniSettlementConverter));
                                }
                        }
