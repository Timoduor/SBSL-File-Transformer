...
{
    public class DailyElmaOmniConverterJob : ConverterJobBase, IHostedService
    {
        private ILogger<DailyElmaOmniConverterJob> _logger;
        IServiceScopeFactory _serviceScopeFactory;
        EmailSender _emailSender;
        private static SemaphoreSlim _semaphore;
        Timer _timer;

        public DailyElmaOmniConverterJob(ILogger<DailyElmaOmniConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            _logger = logger;
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(async state => await MonthlyOmniConverter(), null, TimeSpan.FromSeconds(new Random().Next(15, 60)), TimeSpan.FromMinutes(10));
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    foreach (var file in files)
                    {
                        if (file.ToLower().Contains("utilities") && file.ToLower().Contains("mobile") && file.ToLower().Contains("banking")
                            && file.ToLower().Contains("imke") && (file.ToLower().Contains("omni") || file.ToLower().Contains("elma")) && file.ToLower().Contains("daily"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    await EmailHelpers.SendEmails(dbContext, "Problem Converting Daily Elma Omni Settlement files", $"{file} \n\n {ex.Message}", new string[] { file }, _emailSender);
                                }
                                finally
                                {
