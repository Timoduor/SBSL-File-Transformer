...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class MpesaNewLineCharRemoverJob : ConverterJobBase<MpesaNewLineCharRemoverJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting NewLine Remover Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertC2BFile(), null,
        ...
        private async Task ConvertC2BFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running NewLine Remover job");
                    ...
                    foreach (string file in files)
                    {
                            ...
                            file.ToLower().Contains("imke") && !file.ToLower().Contains("conv"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(MpesaCharRemover));
                                }
                        }
                    }
