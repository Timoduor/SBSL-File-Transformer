...
using SbslFileTransformer.Converters.Kenya.KenSwitch;
...
{
    public class KenSwitchConverterJob : ConverterJobBase<KenSwitchConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting KenSwitch Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertKenSwitchPdfs(), null,
        ...
        private async Task ConvertKenSwitchPdfs()
        {
            try
            {
                ...
                this._logger.LogInformation("Running KenSwitch converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    KenSwitchConverter converter = new KenSwitchConverter();
                    ...
                    {
                        if (file.ToLower().Contains("kenswitch") && file.ToLower().Contains("imke"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(KenSwitchConverter));
                                }
                        }
                    }
