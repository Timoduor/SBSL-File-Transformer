...
{
    public class WesternUnionActivitiesKEConverterJob : ConverterJobBase<WesternUnionActivitiesKEConverterJob>, IHostedService
    {
        public WesternUnionActivitiesKEConverterJob(ILogger<WesternUnionActivitiesKEConverterJob> logger, IServiceScopeFactory serviceScopeFactory, EmailSender emailSender)
        {
            _logger = logger;
        ...
        private async Task WesternUnionActivitiesConverter()
        {
            try
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    {
                        if (!file.Contains("Conv") && file.ToLower().Contains("imke") && file.ToLower().Contains("western_union") && file.ToLower().Contains("activities"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    await EmailHelpers.SendEmails(dbContext, "Problem Converting Western Union Activities KE files", $"{file} \n\n {ex.Message}", new string[] { file }, _emailSender);
                                }
                                finally
                                {
