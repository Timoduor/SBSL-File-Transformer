...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class OmniLookupConverterJob : ConverterJobBase<OmniLookupConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting Omni Lookup Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.OmniLookupJobConverter(), null,
        ...
        private async Task OmniLookupJobConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Omni Lookup Converter job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    OmniLookupConverter mpesaConverter = new OmniLookupConverter();
                    ...
                    {
                        if (file.ToLower().Contains("mobile") && file.ToLower().Contains("banking") &&
                            !file.ToLower().Contains("conv")
                            && file.ToLower().Contains("imke") && file.ToLower().Contains("omni") &&
                            file.ToLower().Contains("lookup"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(OmniLookupConverter));
                                }
                        }
                    }
