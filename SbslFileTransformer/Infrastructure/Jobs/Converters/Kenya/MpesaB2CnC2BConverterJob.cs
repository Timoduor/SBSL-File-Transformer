...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class MpesaB2CnC2BConverterJob : ConverterJobBase<MpesaB2CnC2BConverterJob>, IHostedService
    {
        ...
        {
            this._logger.LogInformation("Starting MPesa B2C Converter Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.ConvertMpesaB2CnC2BFile(), null,
        ...
        private async Task ConvertMpesaB2CnC2BFile()
        {
            try
            {
                ...
                this._logger.LogInformation("Running MPesa B2C n C2B converter job");
                    ...
                    foreach (string file in files)
                    {
                            ...
                            )
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(MpesaB2CnC2BConverter));
                                }
                        }
                    }
