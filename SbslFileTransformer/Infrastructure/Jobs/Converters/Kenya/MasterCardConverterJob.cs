...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class MasterCardConverterJob : ConverterJobBase<MasterCardConverterJob>, IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            this._timer = new Timer(async state => await this.ConvertMasterCardFile(), null,
        ...
        private async Task ConvertMasterCardFile()
        {
            try
            {
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    List<string> files = Directory.GetFiles(prodFolder, "*.a024", options).ToList();

                    files.AddRange(Directory.GetFiles(sbFolder, "*.a024", options));

                    MasterCardConverter masterCardConverter = new MasterCardConverter();
                    ...
                    {
                        if (file.ToLower().Contains("mastercard") && file.ToLower().Contains("imke"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(MasterCardConverter));
                                }
                        }
                    }
