...
namespace SbslFileTransformer.Infrastructure.Jobs.Converters.Kenya
{
    public class WesternUnionSettlementKEConverterJob : ConverterJobBase<WesternUnionSettlementKEConverterJob>,
        IHostedService
    {
        protected override string JobName { get; set; } = nameof(WesternUnionSettlementKEConverterJob);
        ...
        {
            this._logger.LogInformation("Starting Western Union Settlement KE Job");

            _semaphore = new SemaphoreSlim(1, 1);

            this._timer = new Timer(async state => await this.WesternUnionSettlementConverter(), null,
        ...
        private async Task WesternUnionSettlementConverter()
        {
            try
            {
                ...
                this._logger.LogInformation("Running Western Union Settlement KE job");
                ...
                using (IServiceScope scope = this._serviceScopeFactory.CreateScope())
                {
                    ...
                    WesternUnionSettlementKEConverter westernUnionConverter = new WesternUnionSettlementKEConverter();
                    ...
                    foreach (string file in files)
                    {
                            ...
                            file.ToLower().Contains("western_union") && file.ToLower().Contains("settlement"))
                        {
                            ...
                            if (fileToProcess != null && fileToProcess.Converted == false)
                                ...
                                {
                                    this.CompleteFileProcessing(updatedFiles, fileToProcess, nameof(WesternUnionSettlementKEConverter));
                                }
                        }
                    }
