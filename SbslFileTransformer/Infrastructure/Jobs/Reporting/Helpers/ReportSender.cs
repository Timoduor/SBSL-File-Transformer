    ...
    public class ReportSender
    {
        ...
        public async Task SendAndSaveReportsToDb(List<(ReportModel, Dictionary<int, string>)> newlyProcessedReports, IProgress<int> sendReportProgress)
        {
            using (IServiceScope scope = this.serviceScopeFactory.CreateScope())
            {
                ...
                foreach ((ReportModel, Dictionary<int, string>) results in newlyProcessedReports)
                {
                    ...
                    if (results.Item2.Count > 0)
                    {
                        foreach (KeyValuePair<int, string> key in results.Item2)
                        {
                            ...
                            IEnumerable<string> emails = GetEmails(key.Key, report, this.serviceScopeFactory);

                            //ONLY SEND EMAILS IF FILE HAS 1 OR MORE RECORDS
                            await emailSender.SendMessage(emails, this.reportConfiguration.EmailHeader + $" Report ID: {report.ReportId}", this.reportConfiguration.EmailBody + Environment.NewLine + $"{key.Key} Days overdue" +
                                                                                                                                           Environment.NewLine +
                    ...
                    else
                    {
                        ...
                        foreach (int r in results.Item1.DaysRange)
                        {
                            ...
                            {
                                outputFile = await this.AdjustBalanceValue(results.Item1.TempReportPath);
                            }

                            IEnumerable<string> emails = GetEmails(r, report, this.serviceScopeFactory);

                            await emailSender.SendMessage(emails, this.reportConfiguration.EmailHeader, this.reportConfiguration.EmailBody + Environment.NewLine +
                                                                                                        $"Report Name {report.Name}" + Environment.NewLine +
                                                                                                        //$"Report for {country} for {sprint}" + Environment.NewLine +
                                                                                                        $"Report generated by: {report.Creator}" + Environment.NewLine +
                    ...
                    {
                        Format = this.reportConfiguration.ExportType,
                ...
                await this.SaveToDb(reportsToSend);
            }
        }
        ...
        private async Task<string> AdjustBalanceValue(string inputFile)
        {
            ...
            string outputFilePath =
                Path.Combine(await FileHelpers.GetTempPath(this.serviceScopeFactory), "Adj_" + inputFileName);
