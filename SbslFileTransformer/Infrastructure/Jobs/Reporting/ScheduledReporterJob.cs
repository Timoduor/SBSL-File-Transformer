...
namespace SbslFileTransformer.Infrastructure.Jobs
{
    public class ScheduledReporterJob : ConverterJobBase<ScheduledReporterJob>, IHostedService
    {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer = new Timer(async (state) => await ProcessNewReports(), null, TimeSpan.FromSeconds(new Random().Next(30, 60)), TimeSpan.FromMinutes(10));
        ...
        private async Task ProcessNewReports()
        {
            ...
            try
            {
                ...
                var config = ReportHelpers.GetConfiguration(_serviceScopeFactory);

                _logger.LogInformation($"Fetching tokens for {config.UserNamesAndPasswords.Count} users");

                var tokens = await ReportHelpers.GetLoginTokens(config, _logger);
                ...
                foreach (var token in tokens)
                {
                    ...
                    var allReports = (await ReportHelpers.GetRecentReports(config, token, _logger)).ToList();
                    ...
                    using (var scope = _serviceScopeFactory.CreateScope())
                    {
                        ...
                        foreach (var report in allReports)
                        {
                            ...
                            var daysRange = ReportHelpers.GetEmailGroupDays(emailGroups, report.Country, report.Sprint, report.Category);
        ...
        private async Task DownloadReportAndSendEmails(ReportConfigModel config, string token, ApplicationDbContext dbContext, ReportModel report, string reportPath, int[] daysRange)
        {
            try
            {
                if (await ReportHelpers.DownloadReport(report.ReportId, config, reportPath, token, _logger))
                {
                    ...
                    if (results.Item2.Count > 0)
                    {
                        foreach (var key in results.Item2)
                        {
                            ...
                            var emails = ReportHelpers.GetEmails(key.Key, report.Country, report.Sprint, report.Category, _serviceScopeFactory);
                            ...
                            await _emailSender.SendMessage(emails, config.EmailHeader + $" Report ID: { report.ReportId }",
                                ...
                                $"Report for {report.Country} for {report.Sprint}" + Environment.NewLine +
                    ...
                    else
                    {
                        ...
                        {
                            await _emailSender.SendMessage(ReportHelpers.GetEmails(r, report.Country, report.Sprint, report.Category, _serviceScopeFactory), config.EmailHeader,
                                config.EmailBody + Environment.NewLine +
                                    $"Report for {report.Country} for {report.Category}" + Environment.NewLine +
                                    $"Report generated by: {report.Creator}" + Environment.NewLine +
        ...
        private async Task<(string, Dictionary<int, string>)> ProcessReportFile(string inputFile, int[] daysRange)
        {
            ...
            {
                return (agingExcel, await ReportHelpers.CreateCsvFile(daysRecordsPairs, _serviceScopeFactory));
            }
            else
            {
        ...
        private async Task<string> CreateModifiedAgingExcel(string inputFile, int[] daysRange)
        {
            ...
            using (var package = new ExcelPackage(new FileInfo(inputFile)))
            {
                ...
                try
                {
                    ...
                    maxDate = ReportHelpers.FromExcelSerialDate(maxDateInt);
                }
                catch (Exception ex)
                {
                ...
                for (var i = start.Row + 7; i <= end.Row; i++)
                {
                    ...
                    {
                        var outputDate = ReportHelpers.FromExcelSerialDate(dateInt);
