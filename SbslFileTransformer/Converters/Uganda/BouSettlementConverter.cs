...
{
    public class BouSettlementConverter
    {
        private readonly ILogger _logger;

        public BouSettlementConverter(ILogger logger)
        {
            _logger = logger;
        ...
        public void ConvertFile(string inputFile, string outputFile = null)
        {
            ...
            using (var reader = new StreamReader(inputFile))
            {
                ...
                using (var csv = new CsvReader(reader, readerConfig))
                {
                    while (csv.Read())
                    {
                        ...
                        if (!string.IsNullOrEmpty(csv.GetField<string>(7)?.ToString()))
                        {
                            row.Col10 = "-" + csv.GetField<string>(7)?.ToString();
                        }
                        else
                        {
                            row.Col10 = csv.GetField<string>(8)?.ToString();
                        }
            ...
            if (string.IsNullOrEmpty(outputFile))
            {
                ...
                outputFile = Path.Combine(outputFolder,
                    $"{DateTime.Now:yyyy_MM_dd_HH_mm_ss}_BOU_{fileName.Substring(Math.Max(0, fileName.Length - 14)).Replace(" ", "")}.csv");
            }
