...
{
    public partial class VisionRecordMatcher
    {
        ...
        public async Task MatchFiles(string finacleFile, string outputPath, VisionRecordType visionRecordType)
        {
            ...
            List<FinacleRec> finacleRecords = await Task.Run(() => this.GetRecordsFromFinacleFile(finacleFile));
            ...
            List<GLPostingRecordBase> unmatchedVisionRecords = await this.GetUnmatchedVisionRecords();
            ...
            foreach (Tuple<string, string> finRef in finacleRefs)
            {
                ...
                List<GLPostingRecordBase> matchedRecords = new List<GLPostingRecordBase>();
                ...
                List<GLPostingRecordBase> matchedRecs = unmatchedVisionRecords.Where(v => v.ReferenceNumber == finRef.Item1 && v.AccountNumber == finRef.Item2).ToList();
        ...
        private async Task UpdateVisionRecords(VisionRecordType visionRecordType, List<GLPostingRecordBase> matchedRecords)
        {
            this._dbContext.VisionRecordCollections.UpdateRange(VisionCommonHelpers.ConvertParentToChild<GLPostingRecordBase, VisionRecordCollection>(matchedRecords));
        ...
        private async Task CreateFileForReferenceNumber(IEnumerable<GLPostingRecordBase> matchedRecs, string referenceNumber, string accountNumber, string outputPath, VisionRecordType visionRecordType)
        {
        ...
        private async Task<List<GLPostingRecordBase>> GetUnmatchedVisionRecords()
        {
            List<GLPostingRecordBase> visionRecords = await this._dbContext.VisionRecordCollections.AsNoTracking().Where(v => v.Matched == false)
                .Select(r => (GLPostingRecordBase)r).ToListAsync();

            return visionRecords;
        }

        private async Task GenerateFileForSelectedRecords(IEnumerable<GLPostingRecordBase> rows, string outputFile)
        {
            ...
            using (FileStream stream = File.Open(cmsFile, FileMode.Open, FileAccess.Read))
            {
                ...
                using (reader)
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            _logger.LogError(ex, $"Error reading Finacle file {cmsFile}");
                        }
                    }
                }
                stream.Close();
