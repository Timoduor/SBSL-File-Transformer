...
namespace SbslFileTransformer.Converters.Rwanda.Camt053
{
    public class Mt320Converter
    {
        public void ProcessMt320File(string file, string outputFolder = null)
        {
            ...
            if (content.Split(':')[6].Trim().Substring(0, 7) != "FIN 320")
            {
                var archive = "";
            ...
            if (sDet.Length != 0)
            {
                ...
                var l = GetRtgsDetails_MT320_15A(sDet);
                var lb = GetRtgsDetails_MT320_15B(sDet);
                var lc = GetRtgsDetails_MT320_15C(sDet);
                var ld = GetRtgsDetails_MT320_15D(sDet);
                ...
                seq15A.ScopeofOperation15A = l.Any(p => p.StartsWith("94A:"))
                    ...
                    : "";
                seq15A.TypeofEvent15A = l.Any(p => p.StartsWith("22B:"))
                    ? l.First(p => p.StartsWith("22B:")).Split('|')[1].ToString()
                    : "";
                ...
                seq15B.NewSequenceB = "";
                seq15B.PartyAsRole15B = lb.Any(p => p.StartsWith("17R:"))
                    ? lb.First(p => p.StartsWith("17R:")).Split('|')[1].ToString()
                    : "";
                ...
                seq15B.ValueDate15B = lb.Any(p => p.StartsWith("30V:"))
                    ...
                    : "";
                seq15B.MaturityDate15B = lb.Any(p => p.StartsWith("30P:"))
                    ? lb.First(p => p.StartsWith("30P:")).Split('|')[1].ToString().Replace(',', '.')
                    : "";
                seq15B.CurrencyPrincipalAmount15B = lb.Any(p => p.StartsWith("32B:"))
                    ? lb.First(p => p.StartsWith("32B:")).Split('|')[1].ToString().Split(':')[1].Trim().Substring(0, 3)
                    : "";
                seq15B.PrincipalAmount15B = lb.Any(p => p.StartsWith("32B:"))
                    ? lb.First(p => p.StartsWith("32B:")).Split('|')[2].ToString().Trim().Split(',')[1]
                        ...
                        : lb.First(p => p.StartsWith("32B:")).Split('|')[2].ToString().Split(':')[1].Trim().Split(',')
                          ...
                          lb.First(p => p.StartsWith("32B:")).Split('|')[2].ToString().Split(':')[1].Trim()
                              ...
                              .Replace("#", "")
                    : "";
                seq15B.CurrencyAmounttobeSettled15B = lb.Any(p => p.StartsWith("32H:"))
                    ? lb.First(p => p.StartsWith("32H:")).Split('|')[1].ToString().Split(':')[1].Trim().Substring(0, 3)
                    : "";
                seq15B.AmounttobeSettledt15B = lb.Any(p => p.StartsWith("32B:"))
                    ? lb.First(p => p.StartsWith("32B:")).Split('|')[2].ToString().Trim().Split(',')[1]
                        .Replace("#", "") == ""
                        ?
                        lb.First(p => p.StartsWith("32H:")).Split('|')[2].ToString().Split(':')[1].Trim().Split(',')[0]
                            .Replace("#", "")
                        : lb.First(p => p.StartsWith("32H:")).Split('|')[2].ToString().Split(':')[1].Trim().Split(',')
                              [0].Replace("#", "") + "." +
                          lb.First(p => p.StartsWith("32H:")).Split('|')[2].ToString().Split(':')[1].Trim()
                              .Split(',')[1]
                              .Replace("#", "")
                    : "";
                seq15B.NextInterestDueDate15B = lb.Any(p => p.StartsWith("30X:"))
                    ? lb.First(p => p.StartsWith("30X:")).Split('|')[1].ToString()
                    : "";
                seq15B.CurrencyInterestAmount15B = lb.Any(p => p.StartsWith("34E:"))
                    ? lb.First(p => p.StartsWith("34E:")).Split('|')[1].ToString().Split(':')[1].Trim().Substring(0, 3)
                    : "";
                seq15B.InterestAmount15B = lb.Any(p => p.StartsWith("34E:"))
                    ? lb.First(p => p.StartsWith("34E:")).Split('|')[2].ToString().Trim().Split(',')[1]
                        .Replace("#", "") == ""
                        ?
                        lb.First(p => p.StartsWith("34E:")).Split('|')[2].ToString().Split(':')[1].Trim().Split(',')[0]
                            .Replace("#", "")
                        : lb.First(p => p.StartsWith("34E:")).Split('|')[2].ToString().Split(':')[1].Trim().Split(',')
                              [0].Replace("#", "") + "." +
                          lb.First(p => p.StartsWith("34E:")).Split('|')[2].ToString().Split(':')[1].Trim()
                              .Split(',')[1]
                              .Replace("#", "")
                    : "";
                seq15B.InterestRate15B = lb.Any(p => p.StartsWith("37G:"))
                    ? lb.First(p => p.StartsWith("37G:")).Split('|')[1].ToString().Replace(',', '.')
                    : "";
                seq15B.DayCountFraction15B = lb.Any(p => p.StartsWith("14D:"))
                    ? lb.First(p => p.StartsWith("14D:")).Split('|')[1].ToString()
                    : "";
                seq15B.LastDayoftheFirstInterestPeriod15B = lb.Any(p => p.StartsWith("30F:"))
                    ? lb.First(p => p.StartsWith("30F:")).Split('|')[1].ToString()
                    : "";
                seq15B.NumberofDays15B = lb.Any(p => p.StartsWith("38J:"))
                    ? lb.First(p => p.StartsWith("38J:")).Split('|')[1].ToString()
                    : "";


                seq15C.NewSequenceC = "";
                seq15C.ReceivingAgent15C = lc.Any(p => p.StartsWith("57A:"))
                    ? lc.First(p => p.StartsWith("57A:")).Split('|')[1].ToString()
                    : "";

                seq15D.NewSequenceD = "";
                seq15D.DeliveryAgent15D = ld.Any(p => p.StartsWith("53A:"))
                    ? ld.First(p => p.StartsWith("53A:")).Split('|')[1].ToString()
                    : "";
                seq15D.ReceivingAgent15D = ld.Any(p => p.StartsWith("T57A:"))
                    ? ld.First(p => p.StartsWith("T57A:")).Split('~')[1].ToString()
                    : "";
                seq15D.BeneficiaryInstitution15D = ld.Any(p => p.StartsWith("T58A:"))
                    ? ld.First(p => p.StartsWith("T58A:")).Split('~')[1].ToString()
                    : "";


                scontent =
                    " New Sequence A, Sender's Reference,Related Reference,Type of Operation,Scope of Operation,Type of Event,Common Reference ,  " +
                    ...
                    " New Sequence D,Delivery Agent - FI BIC,Receiving Agent - FI BIC,Beneficiary Institution - BIC " +
                    Environment.NewLine;
                scontent += seq15A.NewSequenceA + "," + seq15A.SenderRef15A + "," + seq15A.RelatedRef15A + "," +
                            seq15A.TypeofOperation15A + "," + seq15A.ScopeofOperation15A + "," + seq15A.TypeofEvent15A +
                            "," + seq15A.CommonReference15A + "," + seq15A.PartyA15A + "," + seq15A.PartyB15A;
                scontent += "," + seq15B.NewSequenceB + "," + seq15B.PartyAsRole15B + "," + seq15B.TradeDate15B + "," +
                            seq15B.ValueDate15B + "," + seq15B.MaturityDate15B + "," +
                            seq15B.CurrencyPrincipalAmount15B + "," + seq15B.PrincipalAmount15B + "," +
                            seq15B.CurrencyAmounttobeSettled15B + "," + seq15B.AmounttobeSettledt15B + "," +
                            seq15B.NextInterestDueDate15B + "," + seq15B.CurrencyInterestAmount15B + "," +
                            seq15B.InterestAmount15B + "," + seq15B.InterestRate15B + "," + seq15B.DayCountFraction15B +
                ...
                scontent += "," + seq15D.NewSequenceD + "," + seq15D.DeliveryAgent15D + "," + seq15D.ReceivingAgent15D +
                            "," + seq15D.BeneficiaryInstitution15D;
                WriteFile(outputFolder + "\\Converted_MT320_" + Path.GetFileNameWithoutExtension(file) + ".csv",
                    scontent);
            }
        }
        ...
        private List<string> GetRtgsDetails_MT320_15A(string[] d)
        {
            ...
            for (var i = 1; i < d.Length - 1; i++)
            {
                ...
                if (d[i].Contains("21:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
        ...
        private List<string> GetRtgsDetails_MT320_15B(string[] d)
        {
            ...
            for (var i = 1; i < d.Length - 1; i++)
            {
                ...
                if (d[i].Contains("17R:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("30T:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("30V:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());

                if (d[i].Contains("30P:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("32B:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim() + "|" + d[i + 2].Trim());
                if (d[i].Contains("33B:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim() + "|" + d[i + 2].Trim());
                if (d[i].Contains("32H:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim() + "|" + d[i + 2].Trim());
                if (d[i].Contains("30X:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("34E:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim() + "|" + d[i + 2].Trim());
                if (d[i].Contains("37G:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("14D:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
                if (d[i].Contains("30F:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());

                if (d[i].Contains("38J:")) l.Add(d[i].Trim() + "|" + d[i + 1].Trim());
            }
        ...
        private List<string> GetRtgsDetails_MT320_15D(string[] d)
        {
            ...
            for (var i = 1; i < d.Length - 1; i++)
            {
                if (d[i].Contains("15D:")) l.Add(d[i].Trim());
